...
class Scoring(torch.nn.Module):
    ...
    def forward(self, v, a, v_len, is_residual=True, is_scoring=True):
        if not is_scoring:
            ...
            reliability_scores_vid_s = vid_s
            if is_residual:
                vid_s = v * vid_s + v
            ...
            reliability_scores_aud_s = aud_s
            if is_residual:
                aud_s = a * aud_s + a
            elif not is_residual:
                aud_s = a * aud_s
            ...
            reliability_scores = torch.cat([reliability_scores_vid_s, reliability_scores_aud_s], dim=1)
            merged_attention_mask = self.generate_key_mask(v_len, vid_s.size(1)).to(out.device)
            return out, ~merged_attention_mask.unsqueeze(1).contiguous(), reliability_scores_vid_s.mean(dim=-1)
