...
class AVDataset(Dataset):
    ...
    def build_file_list(self, split_file, mode, data_type):
        ...
        if mode == 'val':
            ...
            elif data_type == 'voxceleb':
                return [os.path.join('dev', x.strip().split()[0]) for x in datalist]
            else:
    ...
    def __getitem__(self, idx):
        ...
        #print(aud.shape)
        #print(int(len(vid) / 25 * 16000) - len(aud))
        #print(np.zeros(int(len(vid) / 25 * 16000) - len(aud)).shape)
        if len(aud) < int(len(vid) / 25 * 16000):
            aud = np.concatenate([aud, np.zeros(int(len(vid) / 25 * 16000) - len(aud))])
        ...
        if self.visual_corruption:
            ...
            if prob < 0.2:
                ...
                vid, _, noise_mask_occ = self.visual_corruption.occlude_sequence(vid, lm, yx_min, freq=freq1)
                vid, noise_mask_blur = self.visual_corruption.noise_sequence(vid, freq=freq2)
                noise = noise_mask_occ + noise_mask_blur
        
        vid = self.transform_vid(vid)
        aud = self.transform_aud(aud)


        return vid, aud, text, noise
    ...
    def collate_fn(self, batch):
        ...
        for i, data in enumerate(batch):
            if data is not None:
                ...
                if noise!=None:
                    padded_noise.append(torch.cat([torch.tensor(noise.tolist()), torch.zeros([max_vid_len - len(noise)])], 0))
                else:
                    padded_noise.append(torch.zeros([max_vid_len]))
        ...
        if padded_noise == []:
            ...
            noise = torch.stack(padded_noise, 0).float()
        vid_len = torch.IntTensor(vid_len)
        aud_len = torch.IntTensor(aud_len)
        noise_len = torch.IntTensor(noise_len)
        return vid, aud, vid_len, aud_len, text, noise, noise_len
