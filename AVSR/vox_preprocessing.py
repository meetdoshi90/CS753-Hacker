...
def build_file_list(data_path, data_type):
    if data_type == 'LRS2':
        ...
        files = sorted(glob.glob(os.path.join(data_path, 'dev', '*', '*', '*.txt')))
    else:
...
def per_file(f, args, video_process):
    ...
    txt_name = os.path.join(args.text_path, f + '.txt')
    if not os.path.exists(txt_name):
        return #return if text transcript does not exists
    ...
    if os.path.exists(os.path.join(args.landmark_path, f + '.pkl')):
        ...
        vid_name = os.path.join(args.text_path.replace('txt_dir','vid_dir'), f + '.mp4')
        ...
        shutil.copy(vid_name.replace('vid_dir','aud_dir')[:-4] + '.wav', aud_save_path + '.wav')
        shutil.copy(vid_name.replace('vid_dir','txt_dir')[:-4] + '.txt', txt_save_path + '.txt')

def main():
    parser = get_parser()
    args = parser.parse_args()
    video_process = VideoProcess(mean_face_path='20words_mean_face.npy', convert_gray=False)
    file_lists = build_file_list(args.text_path, args.data_type)
    #print(file_lists)
    print(len(file_lists))
    #return
    #file_lists = file_lists[:100]
...
def get_parser():
    ...
    parser.add_argument(
        "--text_path", type=str, required=True, help="path including video"
    )
    parser.add_argument(
