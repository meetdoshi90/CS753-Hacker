...
class E2E(torch.nn.Module):
    ...
    def forward(self, xs_pad, xs_aud_pad, ilens, ilens_aud, ys_pad, noise=None, noise_len=None):
        """E2E forward.
        ...
        if self.architecture in ['VCAFE', 'AVRelScore']:
            hs_pad, hs_mask, fusion_reliability_scores = self.encoders(xs_pad, src_mask, ilens, xs_aud_pad, src_mask_aud)
        ...
        alpha = self.mtlalpha
        if noise != None:
            #print(fusion_reliability_scores.shape,noise.shape)
            fusion_loss = nn.BCELoss()
            #print(noise)
            noise = torch.clamp(1-noise, min=0.05, max=0.95)
            fusion_loss_score = fusion_loss(fusion_reliability_scores, noise)
        if alpha == 0:
            self.loss = loss_att
    ...
    def encode(self, x, x_a, extract_resnet_feats=False):
        ...
        if extract_resnet_feats:
            ...
            if self.architecture in ['VCAFE', 'AVRelScore']:
                enc_output, _, _ = self.encoders(x, None, torch.tensor([x.size(1)]), x_a, None)
            elif self.architecture == 'Conformer':
